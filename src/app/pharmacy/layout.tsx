...
const navItems = [
  { href: "/pharmacy", label: "Verify & Dispense", icon: LayoutDashboard },
  { href: "/pharmacy/history", label: "Dispense History", icon: History },
  { href: "/pharmacy/compliance", label: "Audit Trail", icon: ClipboardCheck },
  { href: "/pharmacy/profile", label: "Profile", icon: UserCircle },
];
...
export default function PharmacyLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (mounted && !loading && (!user || user.role !== "pharmacy")) {
      router.replace("/");
    }
      ...
      <div className="p-5 border-b border-slate-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400">Pharmacy Portal</p>
          </div>
        </div>
      </div>
